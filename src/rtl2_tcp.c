...
static uint32_t ds_temp, ds_threshold = 0;
static	int sideband = 0;
static	int offset_tuning = 0;
static	int gain = 0;
static	float ppm_error = 0;
...
int sendBuffer(SOCKET socket, const char *txbuf, int len, volatile int *do_exit)
{
	...
	while (bytesleft > 0)
	{
		...
		tv.tv_sec = 1;
		tv.tv_usec = 0;
		if (select(socket + 1, NULL, &writefds, NULL, &tv))
		{
			bytessent = send(socket, txbuf+index, bytesleft, 0);
			bytesleft -= bytessent;
			index += bytessent;
		}
...
int selectDevice(uint32_t crc)
{
	...
	for (i = 0; i < numDevices; i++)
	{
		...
		{
			fprintf(stderr, "Device %d with CRC %x selected.\n", i, crc);
...
static void show_adc_level(uint8_t *buf, int len, struct iq_state *iq)
{
	...
	if(iq->plot_count==125)
	{
		...
		{
			fprintf(stderr, "U = %.1f mV = %.2f dBmV", U, dbi);
			iq->dbi = dbi;
		}
		iq->plot_count = 0;
	}
...
void usage(void)
{
	fprintf(stderr, "rtl2_tcp, an I/Q spectrum server for RTL2832 based DVB-T receivers\n"
...
static void iqBalance(uint8_t *buf, int len, struct iq_state *iq)
{
	...
	//fprintf(stderr, "I = %f, Q = %f, I/Q = %f%%, len = %d\n", iq->levelI, iq->levelQ, iq_ratio, len);
	if(iq_ratio > 1.01)
	{
	    for (pos = 0; pos < len; pos+=2)
...
static void *sendStream(void *arg)
{
	...
	while(1)
	{
		...
		{
			pthread_mutex_unlock(&ll_mutex);
			fprintf(stderr, "Tx thread timeout\n");
		...
		while(curelem != 0)
		{
			if (!sendBuffer(s, curelem->data, curelem->len, &do_exit))
			{
#ifdef _WIN32
				fprintf(stderr, "Tx thread terminated (%d), do_exit:%d\n", WSAGetLastError(), do_exit);
#else
				fprintf(stderr, "Tx thread terminated, do_exit:%d\n", do_exit);
#endif
				sighandler(CTRL_CLOSE_EVENT);
				pthread_exit(NULL);
			}
...
static void *receive(void *arg)
{
	...
	while(1)
	{
...
#ifdef _WIN32
				fprintf(stderr, "Rx thread terminated (%d), do_exit:%d\n", WSAGetLastError(), do_exit);
#else
				fprintf(stderr, "Rx thread terminated, do_exit:%d\n", do_exit);
#endif
				sighandler(CTRL_CLOSE_EVENT);
				pthread_exit(NULL);
			}
		...
		case SET_FREQUENCY_CORRECTION_PPB://0x83
			...
			break;
		default:
			fprintf(stderr, "unsupported cmd 0x%0x\n", cmd.cmd);
			break;
		}
		cmd.cmd = 0xff;
	}
	CommState = ST_DEVICE_RELEASED;
	return NULL;
}
...
int main(int argc, char **argv)
{
	...
	socklen_t rlen;
	fd_set readfds;
	uint8_t dongle_info[20];
	pthread_t ctrl_thread; //-cs- for periodically reading the register values
	...
	while(1)
	{
...
#endif
		pthread_create(&ctrl_thread, NULL, &ctrl_thread_fn, &ctrldata);
		...
		memcpy(dongle_info, "RTL0", 4);
		dongle_info[6] = 1; //bitwidth;
		dongle_info[7] = 5; //5:RTL-SDR, 6:RSP1, 7: RSP1A, 8: RSP2
		memcpy(dongle_info+15, "RTL0", 4);
